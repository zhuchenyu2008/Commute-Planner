...
const email = "rus-008@example.com";
const password = "password";
const initialPrompt = "2026-07-03 09:00 arrive at Longhu Tianjie";
const changePrompt =
  "meeting moved to 2026-07-03 10:00, recalculate departure time";
const expectedInitialArriveAt = new Date("2026-07-03T01:00:00.000Z");
const expectedChangedArriveAt = new Date("2026-07-03T02:00:00.000Z");
...
test("RUS-008 recalculates a current trip when arrival time moves later", async ({
  page,
}) => {
  ...
  const initialLeg = initialTrip.legs[0];
  const initialLatestDepartAt = initialLeg?.latestDepartAt;
  const initialReminderIds = initialTrip.reminderJobs.map((job) => job.id);

  expect(initialTrip.targetArriveAt?.toISOString()).toBe(
    expectedInitialArriveAt.toISOString()
  );
  ...
  expect(activeLeg.latestDepartAt!.getTime()).toBeGreaterThan(
